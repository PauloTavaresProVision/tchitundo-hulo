...
export async function POST(request: Request) {
  ...
  if (!challenge || !["mfa", "setup"].includes(challenge.purpose)) return Response.json({ error: "O desafio de autenticação expirou. Inicie novamente." }, { status: 401 });
  ...
  if (!body?.code || !user?.active || !user.mfaSecretEncrypted || !await verifyTotp(user.mfaSecretEncrypted, body.code)) {
    ...
    await recordAudit({ action: "auth.mfa", outcome: "failure", request, userId: challenge.userId, username: user?.username });
  ...
  await recordAudit({ action: "auth.login", outcome: "success", request, userId: updated.id, username: updated.username });
