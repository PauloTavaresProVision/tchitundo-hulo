...
export default function SiteHome({ initialContent, preview = false }: { initialContent: SiteContent; preview?: boolean }) {
  ...
  return (
    <main>
      ...
      {filmOpen && <div className="film-modal" role="dialog" aria-modal="true" aria-label={video.enabled && video.src ? video.title : undefined} aria-labelledby={video.enabled && video.src ? undefined : "film-modal-title"} onClick={() => setFilmOpen(false)}><div className={video.enabled && video.src ? "film-player-modal" : ""} onClick={(event) => event.stopPropagation()}><button type="button" onClick={() => setFilmOpen(false)} aria-label="Fechar">×</button>{video.enabled && video.src ? <video controls autoPlay playsInline preload="metadata" poster={optimizedMediaUrl(video.poster)} src={video.src}>O seu navegador não suporta vídeo HTML5.</video> : <><span className="film-icon" aria-hidden="true">▶</span><p className="eyebrow">{video.type}</p><h3 id="film-modal-title">O filme está em preparação.</h3><p>Este módulo está pronto para receber o documentário e os conteúdos audiovisuais oficiais da campanha.</p></>}</div></div>}
    </main>
  );
}
